...
fileprivate struct NoObservableCounter: View {
    ...
    @StateObject private var model = NoObservableClass() // 使用 @StateObject
...
#Preview("观察全局单例不使用 @State 包装器") {
    Counter(model: Model.shared)
}

// MARK: - Observable 宏
// 1.它会为所附加的类型添加遵守 Observable 标记协议的声明这是⼀个在运⾏时没有实际作⽤在编译时标记⼀个类的空协议
...
// ⚠️这套新模型⽐起之前要简单得多,例如在视图 body 中访问⼀个 (可观察的) 全局单例将会⾃动形成访问属性与视图之间的依赖关系⽽⽆需我们使⽤ @ObservedObject 将此单例传递给视图
...
// 同样如果你没有使⽤模型对象例如当它仅在代码的⼀个分⽀中存在时则根本不会观察这个对象这可以减少很多不必要的视图更新提⾼性能
// MARK: - 常见的错误使用方式
